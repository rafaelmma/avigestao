...
export const stripeWebhook = functions
  ...
  console.log("req.rawBody type:", typeof req.rawBody, "is Buffer:", Buffer.isBuffer(req.rawBody));
...
async function resolveUserIdFromStripe(params: {customerId?: string | null; subscriptionId?: string | null}) {
  ...
  if (subscriptionId) {
    const snapshot = await db.collection("users").where("subscription.stripeSubscriptionId", "==", subscriptionId).limit(1).get();
    if (!snapshot.empty) return snapshot.docs[0].id;
  }
...
export const debugReadUser = functions
  ...
  try {
    ...
    const updatedDoc = await db.collection("users").doc(userId as string).get();
...
export const forcePROStatus = functions
  ...
  try {
    ...
    const settingsRef = db.collection("users").doc(userId as string).collection("settings").doc("preferences");
    ...
    console.log("Settings doc after:", after_settings.data()?.plan, after_settings.data()?.trialEndDate);
...
// ============================================
export const uploadLogo = functions
  .region("southamerica-east1")
