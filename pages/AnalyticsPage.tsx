...
interface VerificationStats {
  ...
  recentVerifications: BirdVerificationRecord[];
}
...
const AnalyticsPage: React.FC = () => {
  ...
  const loadAnalytics = async () => {
    try {
      ...
      // Agrupar por pássaro
      const grouped = new Map<string, BirdVerificationRecord[]>();
      filtered.forEach((v) => {
      ...
      for (const [birdId, verifications] of grouped.entries()) {
        ...
        if (!birdName) {
          try {
            ...
            console.warn('Erro ao buscar nome do pássaro:', birdId, err);
            birdName = `Pássaro ${birdId.substring(0, 8)}`;
          }
        }

        const timestamps = verifications.map((v) =>
          toDateFrom((v as unknown as Record<string, unknown>).timestamp),
        );
        ...
        statsArray.push({
          ...
          recentVerifications: verifications.sort((a, b) => {
            const dateA = toDateFrom((a as unknown as Record<string, unknown>).timestamp);
            const dateB = toDateFrom((b as unknown as Record<string, unknown>).timestamp);
            ...
            <div className="p-6 border-b border-slate-200 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">
                  Cidades de onde foram feitas as verificações
                </p>
              </div>
            </div>
            <div className="p-6">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-600">
                          {loc.count} leitura{loc.count !== 1 ? 's' : ''}
                        </p>
                      </div>
                    </div>
        ...
        <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="p-6 text-center text-slate-500">
              Nenhuma verificação registrada neste período
            </div>
          ) : (
            <div className="divide-y divide-slate-200">
              {stats.map((stat) => (
                <div
                  ...
                  onClick={() =>
                    setSelectedBirdId(selectedBirdId === stat.birdId ? null : stat.birdId)
                  }
                >
                  <div className="flex items-center justify-between">
                        ...
                        <table className="w-full text-xs">
                          ...
                          <tbody className="divide-y divide-slate-100">
                            {stat.recentVerifications.slice(0, 10).map((v, idx) => {
                              const timestamp = toDateFrom(v.timestamp);
                              const loc = v.location;
                              const locationText =
                                loc && loc.city && loc.city !== 'Desconhecida'
