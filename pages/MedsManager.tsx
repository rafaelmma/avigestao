...
const MedsManager: React.FC<MedsManagerProps> = ({ 
  ...
  const handleDeleteClick = (id: string) => { if (deleteMed) deleteMed(id); };
  const handleRestoreClick = (e: React.MouseEvent, id: string) => { e.stopPropagation(); if (restoreMed) restoreMed(id); };
  const handlePermanentDelete = (e: React.MouseEvent, id: string) => { e.stopPropagation(); if (permanentlyDeleteMed) permanentlyDeleteMed(id); };

  const handleDeleteTreatment = (id: string) => { if (deleteTreatment) deleteTreatment(id); };
  const handleRestoreTreatment = (e: React.MouseEvent, id: string) => { e.stopPropagation(); if (restoreTreatment) restoreTreatment(id); };
  const handlePermanentDeleteTreatment = (e: React.MouseEvent, id: string) => { e.stopPropagation(); if (permanentlyDeleteTreatment) permanentlyDeleteTreatment(id); };

  const handleDeleteApp = (id: string) => { if (deleteApplication) deleteApplication(id); };
  const handleRestoreApp = (id: string) => { if (restoreApplication) restoreApplication(id); };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-300">
          ...
          <div className="flex flex-col md:flex-row justify-between gap-4">
             <div className="flex gap-2 bg-white p-1 rounded-xl w-fit border border-slate-100">
                <button 
                  ...
                  className={`px-4 py-2 text-[10px] font-black uppercase rounded-lg transition-all ${currentList === 'active' ? 'bg-slate-100 text-slate-800' : 'text-slate-400'}`}
                >
                  Itens Ativos
                </button>
                <button 
                  onClick={() => setCurrentList('trash')} 
                  className={`px-4 py-2 text-[10px] font-black uppercase rounded-lg transition-all flex items-center gap-1 ${currentList === 'trash' ? 'bg-rose-50 text-rose-500' : 'text-slate-400'}`}
                >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredMeds.map(med => (
              <div key={med.id} className={`group bg-white rounded-[24px] border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col ${currentList === 'trash' ? 'opacity-80' : ''}`}>
            ...
            {currentList === 'active' && (
              ...
              >
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest group-hover:text-brand">Cadastrar Item</span>
              </button>
            )}
          </div>
        </div>
      ...
      {activeTab === 'treatments' && (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300 relative">
           ...
           <div className="flex justify-between items-center bg-indigo-50 p-6 rounded-[24px] border border-indigo-100">
              <div>
                 ...
                 <p className="text-xs text-indigo-700 font-medium mt-1 max-w-xl">
                   Automação de antibióticos e suplementos. Configure uma vez e deixe o sistema gerenciar as doses.
                 </p>
              </div>
              <button 
           ...
           {isBasicPlan && (
             <div className="absolute inset-0 top-[100px] z-10 flex items-start justify-center backdrop-blur-[2px] bg-white/50 pt-10">
                <div className="bg-white rounded-[40px] shadow-2xl p-10 max-w-md text-center border border-indigo-100 relative overflow-hidden">
                   ...
                   <h3 className="text-2xl font-black text-slate-800 tracking-tight">Automação Inteligente</h3>
                   <p className="text-slate-500 font-medium mt-4 leading-relaxed">
                     A gestão automática de tratamentos contínuos com alertas de dosagem é exclusiva do <span className="font-bold text-amber-500">Plano Profissional</span>.
                   </p>
                   <p className="text-xs text-slate-400 mt-2">Evite esquecimentos e garanta a saúde do seu plantel.</p>
           ...
           <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 ${isBasicPlan ? 'opacity-40 pointer-events-none filter blur-[1px]' : ''}`}>
                 ...
                 <div className="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm flex flex-col opacity-70">
                    <div className="flex justify-between items-start gap-3 mb-4">
                       <div className="flex items-start gap-3 flex-1">
                          <div className="w-10 h-10 bg-indigo-50 rounded-xl flex items-center justify-center text-indigo-600"><Syringe size={20} /></div>
                          <div className="pt-0.5">
                              <h4 className="font-black text-slate-800 text-sm">Coccidex (Exemplo)</h4>
                              <p className="text-[10px] font-bold text-slate-400 uppercase mt-0.5">2 gotas • Diário</p>
                          </div>
                       </div>
                       <span className="px-2 py-1 bg-emerald-50 text-emerald-600 text-[9px] font-black uppercase rounded-lg border border-emerald-100 flex items-center gap-1"><PlayCircle size={10} /> Ativo</span>
                    </div>
                    <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 mb-4"><p className="text-[9px] font-bold text-indigo-500 flex items-center gap-1"><Clock size={10} /> Uso Contínuo (5 dias)</p></div>
                    <button className="w-full py-2.5 bg-indigo-600 text-white rounded-xl font-bold text-[10px] uppercase">Registrar Dose</button>
                 </div>
              )}

              {treatmentsList.length > 0 ? treatmentsList.map(treatment => {
                 const med = state.medications.find(m => m.id === treatment.medicationId);
                 const bird = treatment.birdId === 'ALL' ? null : state.birds.find(b => b.id === treatment.birdId);
                 ...
                 const appliedToday = treatment.lastApplicationDate === new Date().toISOString().split('T')[0];

                 return (
                   <div key={treatment.id} className="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm flex flex-col relative overflow-hidden group hover:shadow-md transition-all">
                      ...
                      <div className="flex justify-between items-start gap-3 mb-4">
                         ...
                         <div className="flex items-start gap-3 flex-1 min-w-0">
                            ...
                            <div className="flex-1 min-w-0 pt-0.5">
                                <h4 className="font-black text-slate-800 text-sm truncate leading-tight" title={med?.name}>
                      ...
                      <div className="space-y-3 mb-5">
                         ...
                         <div className="flex items-center gap-2 text-xs font-bold text-slate-600 bg-slate-50 p-2 rounded-xl border border-slate-100">
                            {bird ? (
                              <>
                                <div className="w-5 h-5 rounded-full bg-white flex items-center justify-center shadow-sm"><CheckSquare size={12} /></div>
                                <span className="truncate">{bird.name}</span>
                              </>
                            ) : (
                              <>
                                <div className="w-5 h-5 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center shadow-sm"><LayoutGrid size={12} /></div>
                         ...
                         <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
                            <div className="flex justify-between text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">
                               ...
                               {treatment.endDate && <span>Fim: {new Date(treatment.endDate).toLocaleDateString('pt-BR')}</span>}
                            </div>
                            {treatment.endDate ? (
                              <div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden">
                                 <div 
                                    className={`h-full transition-all duration-500 ${treatment.status === 'Concluído' ? 'bg-emerald-500' : 'bg-indigo-500'}`} 
                                    style={{ width: `${progress}%` }}
                                 ></div>
                              </div>
                            ) : (
                              <p className="text-[9px] font-bold text-indigo-500 flex items-center gap-1"><Clock size={10} /> Uso Contínuo ({daysPassed} dias)</p>
                            )}
                         </div>
                      </div>
                ...
                !isBasicPlan && (
                  <div className="col-span-full py-16 text-center text-slate-300">
                     ...
                     <p className="text-sm font-bold uppercase tracking-widest opacity-60">Nenhum tratamento ativo</p>
                  </div>
                )
              )}
           </div>
           ...
           <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/30">
              <div className="flex items-center gap-3">
                ...
                <div>
                   ...
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{filteredHistory.length} registros</p>
                </div>
              </div>
              ...
              <div className="flex flex-col md:flex-row gap-2">
                  <div className="flex bg-white p-1 rounded-xl shadow-sm border border-slate-100">
                     <button 
                       onClick={() => setCurrentList('active')} 
                       className={`px-3 py-2 text-[10px] font-black uppercase rounded-lg transition-all ${currentList === 'active' ? 'bg-slate-100 text-slate-800' : 'text-slate-400'}`}
                     >
                       Ativos
                     </button>
                     <button 
                       onClick={() => setCurrentList('trash')} 
                       className={`px-3 py-2 text-[10px] font-black uppercase rounded-lg transition-all flex items-center gap-1 ${currentList === 'trash' ? 'bg-rose-50 text-rose-500' : 'text-slate-400'}`}
                     >
                       <Trash2 size={10} /> Lixeira
                     </button>
                  </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredHistory.length > 0 ? filteredHistory.map(app => (
                    <tr key={app.id} className={`hover:bg-slate-50/50 transition-colors group ${currentList === 'trash' ? 'opacity-70' : ''}`}>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                           <Calendar size={14} className="text-slate-300" />
                           <span className="text-xs font-bold text-slate-600">{new Date(app.date).toLocaleDateString('pt-BR')}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-xs text-slate-400 max-w-xs truncate">{app.notes || '-'}</td>
                    ...
                    <tr>
                      <td colSpan={6} className="px-6 py-16 text-center text-slate-300">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <p className="text-sm font-bold opacity-50 uppercase tracking-widest">Nenhum registro encontrado</p>
                        </div>
                      </td>
                    </tr>
                  )}
          ...
          <div className="bg-white rounded-[40px] w-full max-w-md shadow-2xl overflow-hidden">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div>
                 ...
                 <p className="text-xs text-slate-400 font-bold mt-1 uppercase tracking-wider">{editingMed.name}</p>
              </div>
              <button onClick={() => setEditingMed(null)} className="text-slate-400 hover:text-slate-600"><X size={24} /></button>
            </div>
            
            <form onSubmit={handleUpdateMed} className="p-8 space-y-5">
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Nome Comercial</label>
                <input required type="text" className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-brand font-bold text-slate-700" value={editingMed.name || ''} onChange={e => setEditingMed({...editingMed, name: e.target.value})} />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                 ...
                 <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Lote</label>
                    <input type="text" className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-brand font-bold text-slate-700" value={editingMed.batch || ''} onChange={e => setEditingMed({...editingMed, batch: e.target.value})} />
                 </div>
              </div>

              {/* Controle de Estoque com Botões */}
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                 <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 text-center">Ajuste Rápido de Estoque</label>
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Validade</label>
                <input required type="date" className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-brand font-bold text-slate-700" value={editingMed.expiryDate || ''} onChange={e => setEditingMed({...editingMed, expiryDate: e.target.value})} />
              </div>

              <button type="submit" className="w-full py-4 bg-brand text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl shadow-brand/20 mt-2 flex items-center justify-center gap-2 hover:opacity-90 transition-all">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-in fade-in zoom-in-95 duration-200">
          <div className="bg-white rounded-[40px] w-full max-w-md shadow-2xl overflow-hidden">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center">
              ...
              <button onClick={() => setShowApplyMed(false)} className="text-slate-400 hover:text-slate-600"><X size={24} /></button>
            </div>
            <form onSubmit={handleSaveApp} className="p-8 space-y-5">
              <div>
                ...
                >
                  ...
                  {state.birds.filter(b => b.status === 'Ativo').concat(isEditingApp ? (state.birds.find(b => b.id === newApp.birdId) ? [] : [{...state.birds[0], id: newApp.birdId, name: 'Desconhecido'} as any]) : []).map(b => (
                    <option key={b.id} value={b.id}>{b.name} ({b.ringNumber})</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Medicamento</label>
                <select 
                  required 
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-slate-700" 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required type="date" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-slate-700" value={newApp.date} onChange={e => setNewApp({...newApp, date: e.target.value})} />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Dosagem</label>
                  <input required type="text" placeholder="Ex: 5 gotas" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-slate-700" value={newApp.dosage || ''} onChange={e => setNewApp({...newApp, dosage: e.target.value})} />
                </div>
              </div>
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Observações</label>
                <textarea className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none h-20 resize-none font-medium text-slate-700" value={newApp.notes || ''} onChange={e => setNewApp({...newApp, notes: e.target.value})} />
              </div>
              <button type="submit" className="w-full py-4 bg-brand text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl shadow-brand/20 mt-2 flex items-center justify-center gap-2 hover:opacity-90 transition-all">
                 <CheckCircle2 size={18} /> {isEditingApp ? 'Salvar Alterações' : 'Confirmar Aplicação'}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-in fade-in zoom-in-95 duration-200">
          <div className="bg-white rounded-[40px] w-full max-w-md shadow-2xl overflow-hidden">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center">
              ...
              <button onClick={() => setShowAddTreatment(false)} className="text-slate-400 hover:text-slate-600"><X size={24} /></button>
            </div>
            <form onSubmit={handleSaveTreatment} className="p-8 space-y-5">
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Paciente / Alvo</label>
                <select required className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-slate-700" value={newTreatment.birdId || 'ALL'} onChange={e => setNewTreatment({...newTreatment, birdId: e.target.value})}>
                  <option value="ALL">Coletivo (Todo o Plantel)</option>
                  {state.birds.filter(b => b.status === 'Ativo').map(b => (
                    <option key={b.id} value={b.id}>{b.name} ({b.ringNumber})</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Medicamento</label>
                <select required className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-slate-700" value={newTreatment.medicationId || ''} onChange={e => setNewTreatment({...newTreatment, medicationId: e.target.value})}>
                  <option value="">Selecione o item...</option>
                  {state.medications.map(m => (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required type="date" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-slate-700" value={newTreatment.startDate} onChange={e => setNewTreatment({...newTreatment, startDate: e.target.value})} />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Data Fim (Opcional)</label>
                  <input type="date" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-slate-700" value={newTreatment.endDate || ''} onChange={e => setNewTreatment({...newTreatment, endDate: e.target.value})} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                 <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Frequência</label>
                    <select className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-slate-700" value={newTreatment.frequency} onChange={e => setNewTreatment({...newTreatment, frequency: e.target.value as any})}>
                 ...
                 <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Dosagem Padrão</label>
                    <input required type="text" placeholder="Ex: 2 gotas" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-slate-700" value={newTreatment.dosage || ''} onChange={e => setNewTreatment({...newTreatment, dosage: e.target.value})} />
                 </div>
              </div>
              <div className="pt-2">
                 ...
                 <div className="flex gap-2">
                    <button 
                      ...
                      className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${newTreatment.status === 'Ativo' ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-500'}`}
                    >
                      Ativo
                    </button>
                    <button 
                      ...
                      className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${newTreatment.status === 'Pausado' ? 'bg-amber-500 text-white' : 'bg-slate-100 text-slate-500'}`}
                    >
                      Pausado
                    </button>
                    <button 
                      ...
                      className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${newTreatment.status === 'Concluído' ? 'bg-slate-700 text-white' : 'bg-slate-100 text-slate-500'}`}
                    >
                      Concluído
                    </button>
                 </div>
              </div>
              <button type="submit" className="w-full py-4 bg-indigo-600 text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl shadow-indigo-200 mt-2 flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all">
                 <Repeat size={18} /> {isEditingTreatment ? 'Salvar Alterações' : 'Iniciar Tratamento'}
              </button>
            </form>
          </div>
        </div>
