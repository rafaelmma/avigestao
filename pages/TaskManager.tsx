...
import { AppState, MaintenanceTask } from '../types';
import {
  Plus,
  CheckCircle,
  Circle,
...
const TaskManager: React.FC<TaskManagerProps> = ({
  ...
  const activeTaskStepIndex = Math.max(
    0,
    taskWizardStepsBase.findIndex((step) => step.id === currentList),
  );
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 font-medium text-sm mt-1">
            Organize as tarefas diárias e semanais do criatório.
          </p>
        </div>
      </header>
        ...
        <div className="flex flex-wrap gap-2 pb-2">
          ...
          {overdueCount > 0 && (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1 ${
                filterMode === 'late'
      ...
      {currentList === 'trash' && (
        <div className="bg-rose-50 border-l-4 border-rose-500 p-4 rounded-r-xl mb-4">
          ...
          <p className="text-rose-600 text-xs">
            Tarefas excluídas. Restaure se necessário ou apague definitivamente.
          </p>
          <p className="text-rose-600 text-xs mt-1">
            Itens ficam disponíveis por até 30 dias na lixeira antes de serem removidos
            automaticamente.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="px-3 py-1 bg-brand-soft text-brand text-[10px] font-black rounded-lg uppercase">
              {pendingTasks.length}
            </span>
          </div>
            ...
            {pendingTasks.length > 0 ? (
                  ...
                  >
                        ...
                        <button
                          ...
                          className={`transition-all duration-300 hover:scale-110 ${
                            overdue
                              ...
                              : 'text-slate-200 hover:text-emerald-500'
                          }`}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {t.remindMe && (
                            <Bell
                              size={12}
                              className={`${
                                overdue ? 'text-rose-500 animate-pulse' : 'text-amber-400'
                              } fill-current`}
                            />
                          )}
                        </div>
                    ...
                    {currentList === 'active' ? (
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={(e) => handleRestoreClick(e, t.id)}
                          className="p-2 text-emerald-500 hover:text-emerald-700"
                        >
                          <RefreshCcw size={16} />
                        </button>
                        <button
                          onClick={(e) => handlePermanentDelete(e, t.id)}
              ...
              <div className="p-12 bg-emerald-50/30 border-2 border-dashed border-emerald-100 rounded-[32px] text-center">
                ...
                <p className="text-sm font-bold text-emerald-800 opacity-60">
                  {filterMode === 'late'
                    ? 'Ufa! Nenhuma tarefa atrasada.'
                    : `Nenhuma tarefa pendente ${currentList === 'trash' ? 'na lixeira' : ''}!`}
                </p>
              </div>
            )}
          </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="px-3 py-1 bg-slate-100 text-slate-400 text-[10px] font-black rounded-lg uppercase">
          ...
          <div className="space-y-3">
            {completedTasks.slice(0, 8).map((t) => {
              ...
              return (
                <div
                  key={t.id}
                  className={`bg-white/50 p-5 rounded-[24px] border border-slate-50 flex items-center justify-between ${
                    currentList === 'trash' ? 'opacity-80 border-rose-50' : 'opacity-60'
                  }`}
                >
                  <div className="flex items-center gap-4">
                    <button
                      ...
                      disabled={currentList === 'trash'}
                      title="Reabrir tarefa"
                    >
                      <CheckCircle size={24} />
                    </button>
                    <div>
                      <p className="text-sm font-bold text-slate-500 line-through italic">
                        {t.title}
                      </p>
            ...
            <form onSubmit={handleSave} className="p-10 space-y-6">
              ...
              <div>
                ...
                <input
                  required
                  type="text"
                  placeholder="Ex: Fornecer Farinhada Especial"
                    ...
                    {state.birds
                      ...
                      .map((bird) => (
                        <option key={bird.id} value={bird.id}>
                          {bird.name} - {bird.ringNumber} ({bird.species})
                        </option>
                      ))}
                  </select>
                </div>
              ...
              <div
                className={`p-4 rounded-2xl border flex items-center justify-between cursor-pointer transition-all ${
                  newTask.remindMe ? 'bg-amber-50 border-amber-200' : 'bg-slate-50 border-slate-100'
                }`}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p
                      className={`text-sm font-bold ${
                        newTask.remindMe ? 'text-amber-800' : 'text-slate-500'
                      }`}
                    >
                      Quero ser avisado
                    </p>
                ...
                <div
                  className={`w-12 h-7 rounded-full p-1 transition-colors ${
                    newTask.remindMe ? 'bg-amber-500' : 'bg-slate-200'
                  }`}
                >
                  <div
                    className={`w-5 h-5 bg-white rounded-full shadow-sm transition-transform ${
                      newTask.remindMe ? 'translate-x-5' : 'translate-x-0'
                    }`}
                  ></div>
                </div>
              </div>
              ...
              <button
                type="submit"
                className="w-full py-5 bg-[#0F172A] text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl hover:bg-slate-800 transition-all mt-2"
              >
