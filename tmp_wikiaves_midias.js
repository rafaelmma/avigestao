...
function loadRecord($record)
{
	...
	else
	{
		...
		var htmlPreload  = '<div class="m-content"><div class="wa-image-wrapper" style="width:100%"><div class="image-container wa-record-container wa-ok">';	
				htmlPreload += '<img border="0" id="wa-foto" class="img-responsive wa-ok" src="'+auxLink+'">';
...
function printRecord(rec)
{
	...
	if(isFoto)
	{
	  if(isMobile)
		...
		htmlMidia = '<a href="/'+rec.id+'&'+parametros+'" onclick="return false"><img class="img-responsive" src="'+auxLink+'"></a>';	
	}
	else
	{
    ...
    htmlMidia += '<div style="padding: 6px 10px 5px 10px;color: #aaa;font-size: 0.85em;"><div style="float:right">'+rec.dura+'</div>WA'+rec.id+'</div>';		
		htmlMidia += '<div class="d-none d-sm-block" style="margin:13px 10px 10px 10px"><img src="/img/legendasom.svg" style="max-width:220px;max-height:40px;min-height:38px;margin-left:auto;margin-right:auto;display:table" onerror="this.onerror=null; this.src=\'/img/legendasom.png\'"></div>';
		htmlMidia += '<a href="/'+rec.id+'&'+parametros+'" onclick="return false"><img class="img-responsive d-none d-sm-block" src="'+auxLinkQ+'"></a>';
    htmlMidia += '</div>';
		htmlMidia += '<div class="wa-player progression-small2">';
		htmlMidia += '<div class="responsive-wrapper responsive-audio">';
		htmlMidia += '<audio id="wa-player-'+rec.id+'" class="mejs-container svg wikiaves-player progression-single progression-skin progression-minimal-dark progression-audio-player mejs-audio" controls="controls" preload="none">';
	...
	var s = ''; // HTML string
	s += '	<div class="m-portlet fadeInUp animated wa-record m-portlet--rounded wa-record-mobile'+(isFoto?' wa-foto':' wa-som')+'" id="WA'+rec.id+'" href="_midia_detalhes.php?m='+rec.id+'" idmedia="'+rec.id+'" data-target="#wa-detalhes" grande="'+rec.grande+'" >';
	s += htmlMidia;
	s += '		<div class="m-portlet__body wa-nowrap">';
	s += '			<div class="'+classe+' font-poppins"><a class="m-link" href="/wiki/'+rec.sp.idwiki+'">'+nvt+'</a><br><a class="m-link" href="/wiki/'+rec.sp.idwiki+'"><i>'+nome+'</i></a></div>';
	s += '			<div class="author"><a class="m-link" href="/municipio_'+rec.idMunicipio+'">'+rec.local+'</a><br>'+rec.por+' <a class="m-link" href="/perfil_'+rec.perfil+'">'+rec.autor+'</a></div>';
  ...
  if(true)
	{
		...
		$(griditem).imagesLoaded().progress(function(imgLoad, griditem){
...
$(document).ready(function() 
{
  ...
  var isoParam = {
	...
	$(".m-grid").on('touchmove', function() {
		if (($("element").data('bs.modal') || {})._isShown ) { alert(2); return false; }
