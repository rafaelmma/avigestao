...
wikiaves.midia = {
	...
	{	
		...
		$('.wa-botao-curtir a.wa-likelink:not(.wa-ok)').click(function(e){
			var o = $(this).parent();
	...
	{	
    	...
    	$('.wa-botao-curtir a.wa-likes:not(.wa-ok), .wa-botao-indicar a.wa-indics:not(.wa-ok)').click(function(e)
		{		
		  	var o = $(this).parent();
			var idMidia = o.attr('midia');
      		$('#wa_curtidas').modal('show').on('hidden.bs.modal', function () {
	...
	initDenunciar: function() 
	{
		...
		// Botão Denunciar	
		$('.wa-denunciar:not(.wa-ok)').click(
		 function()
		 {
			 $("#wa_modaldenuncia").modal('show').on('hidden.bs.modal', function () {
					if($('#wa-detalhes').length > 0) {
						$('body').addClass('modal-open');
					}
				});
			 $("#wa_modaldenuncia .modal-body").html($(".wa-spinner").html()).load('/getDenunciarAbuso.php');
 			 return false;
		 }
		).addClass('wa-ok');			

		$("#wa_salvardenuncia:not(.wa-ok)").click(function () {
				...
				$.post("/getIncluirSugestaoCorrecao.php", { t: tipoNaoConformidade, r: relato, m: idmidia }, function(data) {
						$('#divAviso').html(data);
						var content = {};
...
var ConfigComentarios={
	configControlesComentario:function($e)
	{
		$e.find('.wa-botao-responder').click(function() {
			...
			$('#wa_listacomentarios .wa-cancelar-resposta').click(function(){ 
				//$('#wa_listacomentarios #wa_novaresposta').remove();
				$('#wa_listacomentarios #wa_novaresposta').slideUp("fast", function() { $('#wa_listacomentarios #wa_novaresposta').remove();});
			});
			...
			$('#wa_listacomentarios .wa-salvar-resposta').click(function(e){
				...
				if(!$(this).hasClass('m-loader'))
				{
					...
					$.post( "getManterComentarioMidia.php", $( "#wa_listacomentarios #comentarioformResposta" ).serialize() )
						.done(function( data ) {
		...
		$e.find('.wa-botao-aceitar').click(function () {
			...
			$.post( "getAceitarSugestaoComentarioMidia.php", {'idMidia': idmidia, 'idComentarioSugestao': idComentario } )
				.done(function( data ) 
				{
		...
		$e.find('.excCom').click(function(e) {
				...
				swal({
					...
					if(result.value)
					{
						...
						$.post( "getExcluirComentarioMidia.php", {'idComentario': idComentario } )
							.done(function( data ) 
							{
	...
	{
		$('.wa-comentario:not(.wa-ok)').each(function()
		{
	...
	init:function()
	{
		...
		$('#enviarComentario').click(function (e) {
			...
			{
				$(this).addClass('m-loader m-loader--light m-loader--left');
				$.post( "getManterComentarioMidia.php", $( "#comentarioform" ).serialize() )
					.done(function( data ) {
		...
		$('#wa_exibirtodos button').click(function () {
			...
			$('#wa_exibirtodos').html('<div class="wa-spinner" style="display:table"><div class="wa-spinner" style="display:none"><div id="spinner1" class="m-spinner m-spinner--danger m-spinner--sm"></div><div id="spinner2" class="m-spinner m-spinner--warning m-spinner--sm"></div><div id="spinner3" class="m-spinner m-spinner--success m-spinner--sm"></div><div id="spinner4" class="m-spinner m-spinner--primary m-spinner--sm"></div></div></div>');				
			
			$.post( "getTodosComentariosMidia.php", {idMidia: idmidia} )
				.done(function( data ) {
