...
const PedigreeTreeNew: React.FC<PedigreeTreeProps> = ({ bird, allBirds, settings }) => {
  ...
  const resolveBirdData = (path: string) => {
    ...
    if (bird.manualAncestors && bird.manualAncestors[path]) {
      return { name: bird.manualAncestors[path], ring: '', sex: 'Desconhecido' as any };
    }
    ...
    if (parentPath) {
      ...
      if (parentBird && parentBird.manualAncestors && parentBird.manualAncestors[lastChar]) {
        return { name: parentBird.manualAncestors[lastChar], ring: '', sex: 'Desconhecido' as any };
      }
    }
  ...
  const getMaxDepth = (path: string = '', currentDepth: number = 0, maxDepth: number = 10): number => {
    if (currentDepth >= maxDepth) return currentDepth; // Limite de segurança
    
    const data = resolveBirdData(path || 'self');
    if (path === 'self' || path === '') {
      // Verifica pai e mãe
      const fatherDepth = bird.fatherId || (bird.manualAncestors && bird.manualAncestors['f']) 
        ? getMaxDepth('f', 1, maxDepth) 
        : 0;
      const motherDepth = bird.motherId || (bird.manualAncestors && bird.manualAncestors['m']) 
    ...
    const fatherDepth = hasFather ? getMaxDepth(fatherPath, currentDepth + 1, maxDepth) : currentDepth;
    const motherDepth = hasMother ? getMaxDepth(motherPath, currentDepth + 1, maxDepth) : currentDepth;
    
    return Math.max(fatherDepth, motherDepth);
  };

  // Função para gerar todas as gerações recursivamente
  const generateGenerations = (maxDepth: number) => {
  ...
  const BirdBox = ({ 
    ...
    const sizes: { [key: number]: { w: string; text: string; ring: string; py: string; px: string } } = {
    ...
    if (!data) {
      return (
        <div className={`${size.px} ${size.py} rounded-lg border-2 border-dashed border-gray-300 bg-gradient-to-br from-gray-50 to-gray-100 ${size.text} text-gray-400 italic text-center ${size.w} shadow-sm`}>
    ...
    const shadowColor = isMale ? 'rgba(59, 130, 246, 0.3)' : isFemale ? 'rgba(236, 72, 153, 0.3)' : 'rgba(148, 163, 184, 0.3)';

    return (
      <div 
        className={`${size.px} ${size.py} rounded-xl text-center ${size.w} shadow-lg transition-all hover:scale-105 hover:shadow-xl ${isMain ? 'ring-4 ring-yellow-400 ring-offset-2' : ''}`}
        style={{ 
          background: bgGradient,
          boxShadow: `0 8px 20px ${shadowColor}, 0 4px 8px rgba(0,0,0,0.1)`
        }}
      >
        <div className={`font-black ${size.text} text-white uppercase tracking-wide drop-shadow-md truncate`}>
          {data.name}
        </div>
  ...
  const generations = generateGenerations(maxDepth);
  ...
  return (
    ...
    >
      ...
      {/* Header Premium */}
      <div className="relative flex items-center justify-between mb-8 pb-6 border-b-4" style={{ borderColor: accent }}>
        <div className="flex items-center gap-4">
          {settings.logoUrl && (
            <div className="h-20 w-20 rounded-2xl border-4 flex items-center justify-center overflow-hidden bg-white shadow-xl" style={{ borderColor: accent }}>
              <img src={settings.logoUrl} alt="Logo" className="h-full w-full object-contain p-2" />
            </div>
          )}
          <div>
            <div className="flex items-center gap-3 mb-1">
              <span className="px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest text-white shadow-lg" style={{ backgroundColor: accent }}>
                Certificado
              </span>
        ...
        <div className="text-center">
          ...
          <p className="text-xs font-black uppercase tracking-wider px-4 py-1.5 rounded-full bg-white shadow-md" style={{ color: accent }}>
            {maxDepth + 1} Gerações
          </p>
        </div>
      </div>
      ...
      <div className="relative grid grid-cols-[280px_1fr] gap-12 items-start py-8">
        ...
        <div className="flex flex-col items-center justify-center gap-4 sticky top-8">
          <div className="relative">
            ...
            <div className="relative px-6 py-3 rounded-2xl shadow-2xl" style={{ 
          ...
          <BirdBox 
            ...
            isMain={true}
          />
          <div className="text-xs font-bold text-gray-600 uppercase tracking-wider">Pássaro Principal</div>
        </div>
        ...
        <div className="grid grid-rows-2 gap-10">
          ...
          {/* LADO PATERNO */}
          <div className="relative pl-8 border-l-4 rounded-l-3xl py-6" style={{ 
            borderColor: '#3b82f6',
            backgroundColor: 'rgba(59, 130, 246, 0.03)'
          }}>
            <svg className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4" width="32" height="32">
              <line x1="0" y1="16" x2="32" y2="16" stroke="#3b82f6" strokeWidth="4"/>
            </svg>
            
            <div className="mb-4 flex items-center gap-2">
              <div className="text-sm font-black uppercase text-blue-900 tracking-wider">👨 LADO PATERNO</div>
          ...
          <div className="relative pl-8 border-l-4 rounded-l-3xl py-6" style={{ 
            ...
            <div className="mb-4 flex items-center gap-2">
              <div className="text-sm font-black uppercase text-pink-900 tracking-wider">👩 LADO MATERNO</div>
      ...
      {/* Rodapé Premium */}
      <div className="relative mt-10 pt-6 border-t-4 flex items-center justify-between" style={{ borderColor: accent }}>
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white shadow-md">
            <div className="w-5 h-5 rounded-full shadow-lg" style={{ background: 'linear-gradient(135deg, #60a5fa 0%, #3b82f6 100%)' }}></div>
            <span className="text-sm font-bold text-gray-700">Macho ♂</span>
          </div>
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white shadow-md">
            <div className="w-5 h-5 rounded-full shadow-lg" style={{ background: 'linear-gradient(135deg, #f472b6 0%, #ec4899 100%)' }}></div>
